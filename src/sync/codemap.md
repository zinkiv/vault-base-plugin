...
3. `SyncEngine.start()` stores plan summary, then for manual and startup sync opens a task-review modal. The user checks download, upload, merge, and delete tasks before they run. Startup sync and auto-sync on an empty vault still convert remote deletes into downloads. Interval/realtime auto-sync keeps the older delete-only confirmations.
